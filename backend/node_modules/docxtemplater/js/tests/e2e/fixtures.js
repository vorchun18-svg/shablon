...
var fixtures = [{
  ...
  it: "should foobar",
  contentText: '{{% {}["constructor" + ""].getPrototypeOf({}).hasOwnProperty = returnsOne %}}',
  options: {
    parser: expressionParser
  },
  delimiters: {
  ...
  scope: {
    returnsOne: function returnsOne() {
      return 1;
    }
  },
  result: null,
  ...
  assertAfter: function assertAfter() {
    var hasO = {}.hasOwnProperty("foo");
    var hasO2 = Object.prototype.hasOwnProperty.call({}, "foo");
  ...
  return _objectSpread(_objectSpread({}, noInternals), {}, {
    ...
    options: {
      parser: expressionParser.configure({
        evaluateIdentifier: function evaluateIdentifier(tag, scope, scopeList, context) {
          var onlyCurrentScope = /^(_\$)(.*)/g;
          if (onlyCurrentScope.test(tag)) {
            tag = tag.replace(onlyCurrentScope, "$2");
            var deepestScope = scopeList[scopeList.length - 1];
  ...
  function isLast(scope, context) {
    ...
    for (var i = context.scopePathItem.length - 1; i >= 0; i--) {
      if (index == null && context.scopeTypes[i] === "array") {
        var length = context.scopePathLength[i];
  ...
  options: {
    ...
    parser: expressionParser.configure({
      evaluateIdentifier: function evaluateIdentifier(name) {
        if (name === "signature") {
          return "{signature}";
        }
      }
